...
// Package k3smtest holds the ONE cross-repo test-support contract k3sm's
...
// notarization/signing identity, or in a two-machine lab — the exact
// vocabulary already enumerated once, per milestone, in the (READ-ONLY here)
...
// rest of this module.
//
// # Skip vs. fatal — the one behavior that makes this more than a helper
// wrapping t.Skip
//
// A test that calls SkipUnless(t, cap) SKIPS when cap is not available in the
// current process. That is correct for a laptop `go test ./...` run, where
// most capabilities (a second Mac, an Apple GPU, a code-signing identity)
// are legitimately absent. But it is WRONG for a lab run that exists
// specifically to prove a capability-gated code path executes: if every
// gated test quietly skips, the run reports green while proving nothing.
//
...
// SkipUnless calls t.Fatal instead of t.Skip — the run cannot silently pass
