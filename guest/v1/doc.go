...
// `vm`-backend pod, in one home.
//
// It carries three things, because they describe one boundary and splitting
...
//
// Both ends are built from k3sm.io/runtimed, which would normally argue for
// plain Go structs. The guest end, however, ships inside the INITRAMFS: an
// independently released, sha256-pinned artifact acquired at install time, not
// a symbol linked into the daemon. A node can therefore run a daemon and an
// initramfs that were not built together, so this boundary is a wire contract
// in fact — and it is treated as one, with the same additive-only discipline
// and buf baseline coverage as runtime/v1.
//
// # Compat posture
//
...
// names. The dev-lab --guest-artifacts-dir override is UNSUPPORTED SKEW. So
// that unsupported skew is a legible rejection rather than silent stream
// corruption, HealthResponse carries api_version (plus additive-only
// capabilities tokens): a host that dials an agent speaking a different
// api_version fails the pod with that stated reason.
//
// # Stability contract
//
